...
class TransactionsList extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    context.watch<TransactionDBProvider>().refresh();
    // Provider.of<TransactionDB>(context).refresh();

    return Consumer<TransactionDBProvider>(
        // valueListenable: TransactionDB.instance.transactionListNotifier,
        builder: (BuildContext ctx, transactionDBProvider, Widget? _) {
      final newList = transactionDBProvider.transactionListNotifier;
      // Sort the transactions by date in descending order
      newList.sort((latest, oldest) => oldest.date.compareTo(latest.date));
      // Group the transactions by date using a Map
      final groupedTransactions = groupBy(newList,
          (transaction) => DateFormat("dd MMM yyyy").format(transaction.date));
      if (newList.isEmpty) {
                ...
                .map((value) => Slidable(
                      ...
                      child: ListTile(
                          ...
                          leading: Stack(children: [
                            ...
                            Positioned.fill(child: Icon(value.category!.icon))
                          ]),
